    ...
    query = """
        SELECT 
            *,
            DATE(
                CASE 
                WHEN pull_timestamp = '2025-07-22T11:53:38.683010'
                THEN TIMESTAMP_SUB(TIMESTAMP(pull_timestamp), INTERVAL 1 DAY)
                ELSE TIMESTAMP(pull_timestamp)
                END
            ) AS pull_date
...
def get_market_cycle_data(ih_property_df):
    ...
    st.dataframe(ih_property_cycle_df)
